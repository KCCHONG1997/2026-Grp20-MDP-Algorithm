...
class Obstacle(CellState):
    ...
    def get_view_state(self, retrying: bool) -> List[CellState]:
        """Constructs the list of CellStates from which the robot can view the symbol on the obstacle.

        Args:
            retrying: Whether this is a retry attempt (uses farther positions)

        Returns:
            List[CellState]: Valid cell states where robot can be positioned to view the symbol
        """
        cells = []
        offset = EXPANDED_CELL * 2
        
        # Direction config: (dx_sign, dy_sign, primary_axis, robot_direction)
        # primary_axis: 'x' means displacement is primarily in x direction
        direction_config = {
            Direction.NORTH: (0, 1, 'y', Direction.SOUTH),   # Robot faces south, offset in +y
            Direction.SOUTH: (0, -1, 'y', Direction.NORTH),  # Robot faces north, offset in -y
            Direction.EAST: (1, 0, 'x', Direction.WEST),     # Robot faces west, offset in +x
            Direction.WEST: (-1, 0, 'x', Direction.EAST),    # Robot faces east, offset in -x
        }
        
        if self.direction not in direction_config:
            return cells
        
        dx_sign, dy_sign, axis, robot_dir = direction_config[self.direction]
        
        # Generate view positions based on primary axis and retrying status
        # Format: (primary_offset, secondary_offsets, penalty)
        if not retrying:
            positions = [
                (1 + offset, [0], 5),                    # Close center position
                (2 + offset, [0], 0),                    # Far center position (preferred)
                (2 + offset, [1, -1], SCREENSHOT_COST),  # Far side positions
            ]
        else:
            positions = [
                (2 + offset, [0], 0),                    # Far center position
                (3 + offset, [0], 0),                    # Very far center position
                (2 + offset, [1, -1], SCREENSHOT_COST),  # Far side positions
            ]
        
        for primary_offset, secondary_offsets, penalty in positions:
            for sec_offset in secondary_offsets:
                if axis == 'y':
                    nx = self.x + sec_offset
                    ny = self.y + dy_sign * primary_offset
                else:  # axis == 'x'
                    nx = self.x + dx_sign * primary_offset
                    ny = self.y + sec_offset
                
                if is_valid(nx, ny):
                    cells.append(CellState(nx, ny, robot_dir, self.obstacle_id, penalty))
...
class Grid:
    ...
    def reachable(self, x: int, y: int, turn: bool = False, pre_turn: bool = False) -> bool:
        ...
        Args:
            x (int): x-coordinate of candidate cell center
            y (int): y-coordinate of candidate cell center
            turn (bool): if evaluating a post-turn position (stricter buffer)
            pre_turn (bool): if evaluating a pre-turn position (stricter buffer)

        Returns:
            bool: True if reachable/safe, False otherwise
        """
        ...
        for ob in self.obstacles:
            # Special-case bypass for bottom-left start corridor
